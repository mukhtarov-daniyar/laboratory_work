...
public class CacheEfficiencyCalculator {
    ...
    private void algorithm1() {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                Color color = memory.getColor(i, j); // Чтение данных из основной памяти
                cache.accessCache(i * M + j, color, false); // Чтение данных из кэша
                cache.accessCache(i * M + j, color, true); // Запись данных в кэш
            }
        }
    }
